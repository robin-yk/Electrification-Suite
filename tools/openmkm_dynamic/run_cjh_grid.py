...
Pressure and closure are fixed (1 atm) and recorded. The feed is a parameter,
defaulting to CH4:CO2 = 1:1, and each feed gets its own output file.

Fixing the feed is what made a 2-D map sufficient: in the PFR data conversion
collapsed onto (T, tau), but selectivity kept a CH4-fraction dependence (+0.38
rank correlation) that only disappears with the feed held. That was the right
call while the surrogate predicted conversion alone. It stops being the right
call once selectivity and yield are targets, because the dependence the fixed
feed hid is exactly the one those targets need. Hence --feed: the map becomes a
stack of 2-D sheets, one per feed, and the feed joins the surrogate features.
...
def base_params(tau_s, closure, feed):
    args = argparse.Namespace(
        ...
        pressure_atm=1.0, residence_time_s=tau_s, feed=feed,
...
def coarse_temperatures(step_c=COARSE_STEP_C):
    n = int(round((T_HI_C - T_LO_C) / step_c))
    return [T_LO_C + step_c * i for i in range(n + 1)]


def refine_temperatures(coarse_rows, fine_step_c=FINE_STEP_C):
...
def main():
    ...
    ap.add_argument("--feed", default="CH4:1, CO2:1",
                    help="Cantera mole-fraction feed string. One feed per output "
                         "file: the resume index keys on (T, tau) alone, so two "
                         "feeds in one file would silently reuse each other's rows.")
    ap.add_argument("--output", type=Path, required=True)
    ap.add_argument("--shard", type=int, default=0)
    ap.add_argument("--shards", type=int, default=1)
    ap.add_argument("--coarse-step-c", type=float, default=COARSE_STEP_C,
                    help="coarse temperature spacing; halving it roughly doubles the map")
    ap.add_argument("--fine-step-c", type=float, default=FINE_STEP_C,
                    help="spacing used only where the coarse pass straddles the ignition cliff")
    args = ap.parse_args()
    ...
    meta = {"generator": "tools/openmkm_dynamic/run_cjh_grid.py",
            ...
            "feed": args.feed, "pressure_atm": 1.0,
            "generated": datetime.date.today().isoformat()}

    def emit(stream, T_C, tau, pass_name):
        key = (round(T_C, 1), round(tau, 6))
        if key not in have:
            p = base_params(tau, args.closure, args.feed)
    ...
    with args.output.open("a") as stream:
        for tau in my_taus:
            rows = [{"T_K": T_C + 273.15,
                    ...
                    for T_C in coarse_temperatures(args.coarse_step_c)]
            coarse_rows[tau] = rows
            print(f"tau={tau:.4g}s coarse done "
                  f"({sum(1 for r in rows if r['ch4_conversion'] > 0.01)} live)")
        for T_C, tau in refine_temperatures(coarse_rows, args.fine_step_c):

...
def solve_one(ct, mech, diluent, plate_c, u_in_m_s, width_m):
    ...
    sim.inlet.mdot = gas.density * u_in_m_s
...
def main():
    ...
    for t_c in args.plate_c:
        for u in args.velocity_cm_s:
            r = solve_one(ct, args.mechanism, args.diluent, t_c, u / 100.0, args.width_mm / 1e3)
            rows.append(r)

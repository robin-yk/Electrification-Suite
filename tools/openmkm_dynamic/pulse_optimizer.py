...
energy-efficiently on FIXED hardware? Both objectives are maximized:
  ...
  q_C2H2 = mdot_C2H2 / Pbar   [kg per kWh of element electricity]
  q_CO   = mdot_CO   / Pbar   [kg per kWh]

with Pbar the cycle-average electric power V^2/R(T) over the on phase.
...
Element power only; PSU, startup and auxiliaries excluded. One-way
coupling: candidates whose chemical duty exceeds EPS_POWER of Pbar are
flagged unsustainable (their prescribed T(t) would sag in reality).
...
from pulse_common import (load_atlas, load_gp, build_rows, corrected_yields,
                          productivities, pareto_max, chem_from_blend,
...
rows = build_rows(nodes, TAUS, FEEDS, blended_column, HF,
...
def point(i):
    return {
        ...
        "chem_W": float(chem_W[i]), "sustainable": bool(sustainable[i]),
        "J_balance": float(J[i]),
    }
...
report = {
    "meta": {
        ...
        "power": "cycle-average electric element power E_cycle/period; PSU, startup and auxiliaries excluded",
        ...
        "scope": "GRI-Mech 3.0 gas-phase CSTR, trained box; no C4+/PAH/coke sink, so the front is GRI-screened; unsustainable flag marks chemical duty above EPS_POWER of element power",
        "eps_power": EPS_POWER,
        ...
        "sustainable_on_front": int(sum(1 for i in front if sustainable[i])),
    },
...
sel = list(dict.fromkeys(
    [int(i) for i in front[np.linspace(0, len(front)-1,
                                       min(N_VERIFY-3, len(front))).astype(int)]]
...
for k, i in enumerate(sel):
    targets.append({
        ...
        "predicted": {"y_c2h2": float(y1[i]), "y_co": float(y2[i]),
                      "q_c2h2_kg_kwh": float(q1[i]), "q_co_kg_kwh": float(q2[i]),
                      "pe_W": float(pe[i]), "mdot_g_s": float(rows[i, COL['mdot_g_s']])}})

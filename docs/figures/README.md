...
the twelve plates, `figure-data.json`, and `index.html` (the page published as
the figure source).
...
Nothing may fall outside the viewBox. A plate that clips is a plate that will
print clipped, and the check is one page render away: measure every element's
bounding rectangle against the viewBox after transforms, not before, since a
rotated axis label reports an untransformed box that always looks out of
bounds.
...
One hue carries one physical role in all twelve plates. The set is the
